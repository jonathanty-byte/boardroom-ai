...
export function formatCostrat(report: CostratReport): string {
  const lines: string[] = [];
  const date = new Date(report.timestamp);
  const dateStr = date.toISOString().slice(0, 10);

  lines.push(`# COSTRAT — ${report.projectName} — ${dateStr}`);
  lines.push("");
  ...
  if (report.frictions.length > 0) {
    ...
    for (const f of report.frictions) {
      ...
      for (const [role, position] of Object.entries(f.positions)) {
        lines.push(`- **${BOARD_MEMBER_NAMES[role as keyof typeof BOARD_MEMBER_NAMES]}**: ${position}`);
      }
      lines.push("");
    }
  }
  ...
  lines.push("");
  lines.push(`*Generated by COMEX Board — ${report.totalDurationMs ? `${(report.totalDurationMs / 1000).toFixed(1)}s` : ""}*`);
